...
export async function fetchStationData(station) {
    ...
    try {
        ...
        if (station.type === 'azuracast') {
            ...
            // 1. FORMATO NOVO: Array de Canais contendo 'playHistories'
            if (Array.isArray(rootData) && rootData.length > 0 && rootData[0].playHistories) {
                // Tenta encontrar o canal correspondente usando o nome ou o URL. Se não encontrar, usa o primeiro.
                ...
                if (targetChannel && targetChannel.playHistories && targetChannel.playHistories.length > 0) {
                    ...
                    // Processar Histórico (History API)
                    if (station.historyApi && station.historyApi !== "" && station.historyApi !== station.api) {
                        try {
                            const histSeparator = station.historyApi.includes('?') ? '&' : '?';
                            const histUrl = `${station.historyApi}${histSeparator}t=${new Date().getTime()}`;
            ...
            // 2. FORMATO ANTIGO: Array direto de tracks
            ...
            if (rootData.itms && Array.isArray(rootData.itms)) {
                const validTracks = rootData.itms.filter(item => item.type === 'SINGLE' && item.tit && item.art);
                
                if (validTracks.length > 0) {
                    const currentItem = validTracks[0];
                    let artUrl = station.defaultArt;
                    
                    if (currentItem.cov && currentItem.cov.startsWith('http')) {
                        artUrl = currentItem.cov;
                    }

                    songObj = {
                        title: currentItem.tit || '',
                        artist: currentItem.art || '',
                        art: artUrl
                    };

                    historyObj = validTracks.map(item => {
                        return {
