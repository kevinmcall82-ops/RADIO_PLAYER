...
export async function fetchItunesData(title, artist) {
    ...
    const fetchPromise = (async () => {
        try {
            ...
            const res = await fetch(`https://itunes.apple.com/search?term=${cleanQuery}&entity=song&limit=20`);
            ...
            const invalidTerms = [
                ...
                'vol.', 'volume', 'lofi', 'lo-fi', 'lullaby', 'compilation', 'Streaming Only', 'Orgullo', 'Throwbacks', 'Y2K'
            ].map(t => t.toLowerCase());
...
export async function fetchStationData(station) {
    ...
    try {
        ...
        if (station.type === 'azuracast') {
            ...
            // Nova integração para APIs baseadas no padrão jamfm JSON
            listeners = 0; // JSON não especifica listeners 
            
            if (Array.isArray(rootData) && rootData.length > 0) {
                // A música atual é o primeiro item do array
                ...
                // Processar histórico (com ou sem History API)
                if (station.historyApi && station.historyApi !== "" && station.historyApi !== station.api) {
                    try {
                        const histSeparator = station.historyApi.includes('?') ? '&' : '?';
                        const histUrl = `${station.historyApi}${histSeparator}t=${new Date().getTime()}`;
                    ...
                    // Se não houver historyApi explícita, usa o restante do array retornado
